...
class PassiveDrawer {
	...
	draw(canvasToDraw,backCanvasToDraw) {
		...
		this.drawDescription(PASSIVE.description);
		this.drawName(PASSIVE.name);
		this.drawTurn(PASSIVE.time, PASSIVE_COLOR);


		ctx = backCanvasToDraw.getContext("2d");
		drawBackBorder(PASSIVE_COLOR);
		ctx.fillStyle = '#fff';
		setFontSize(30);
	...
	drawTurn(turn, color) {
		const turnLabel = TURN_LABELS[turn];

		ctx.save();
		ctx.translate(WIDTH/2-ART_RECT/2, HEIGHT-30-PLATE_HEIGHT);

		drawPlate(0,0,ART_RECT,"#cd7f32", "#a46628");
		setFontSize(24);
		ctx.fillStyle = "#fff";
		ctx.fillText(turnLabel, ART_RECT/2-ctx.measureText(turnLabel).width/2,PLATE_HEIGHT/2+12);
